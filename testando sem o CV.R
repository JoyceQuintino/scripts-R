...
library(rpart)
library(randomForest)
classificador = rpart(formula = morto ~., data = base_treinamento)
#poda = classificador$cptable[which.min(classificador$cptable[ , "xerror"]), "CP"]
#prune(classificador, poda) é a poda original;
rpart.plot::rpart.plot(classificador)
##
classificadorr = randomForest(x = base_treinamento[-15], y = base_treinamento$morto, ntree = 10)
previsoes = predict(classificadorr, newdata = base_teste[-15])
matrizConfusao = table(base_teste$morto, previsoes)
